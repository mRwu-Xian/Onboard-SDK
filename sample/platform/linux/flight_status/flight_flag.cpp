...
float UVA_status::get_Distance(UVA_status::Point one, UVA_status::Point two)
{
    ...
    ERRORLOG(INFO) << "Waypoint mission distance :" << distance;
    return distance;
}
...
bool UVA_status::judge_Flightstatus(UVA_status::Point position){
    ...
    if(judge_Positionstatus(position)){
        ...
        statusInfo.preStatus = ActionStatus::Static;
    }
    else
    {
        status = ActionStatus::Move;
        statusInfo.preStatus = ActionStatus::Move;
    }
    if (statusInfo.curStatus != statusInfo.preStatus)
    {
        if (statusInfo.preStatus == ActionStatus::Static)
        {
            // 从运动到静止
            FlightFlag.store(false);
            statusInfo.curStatus = statusInfo.preStatus;
            ERRORLOG(INFO) << "Flight Status from Move to static"
            return true;
        }
        else
        {   
            ...
            statusInfo.curStatus = statusInfo.preStatus;
            ERRORLOG(INFO) << "Flight Status from static to Move"
            return true;
        }       
    }
    else
    {
        ...
        {
            ERRORLOG(INFO) << "The current state is" << status;
        }
        else
        {
            ERRORLOG(INFO) << "The current state is" << status;
